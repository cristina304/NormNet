...
class H5DataGenerator(object):
    ...
    def process_train_set(self, depth_img, segment_img, gt_file_path, output_file_path, xyz_limit=None, verbose=True):
        '''
        Input:
        ...
        label_trans, label_rot, label_vs, label_class = self._read_label_csv(gt_file_path)
        ...
        obj_ids = np.round(segment_img[:,:,1][segment_img[:,:,2]==1]*(obj_num-1))
        #print(obj_ids,obj_ids.dtype,obj_ids.shape,type(obj_ids))
        obj_ids = obj_ids.astype(np.int16)
        ...
        label_class = label_class[obj_ids]

        labels = np.concatenate([label_trans, label_rot, label_vs.reshape([-1, 1]), obj_ids.reshape([-1, 1]), label_class], axis=-1)
        
        assert points.shape == (self.target_num_point, 3) and labels.shape == (self.target_num_point, 15)
    ...
    def _read_label_csv(self, file_name):
        ...
        label_trans, label_rot, label_vs, label_class= [], [], [], []
        with open(file_name, 'r') as f:
            for line in f.readlines()[1:]:
                ...
                label_class.append(list(map(int, words[-1])))
        #print(label_para)
        ...
        # print(num_obj)
        # exit()
        # assert label_trans.shape == (num_obj, 3)
        # assert label_rot.shape == (num_obj, 9)
        # assert label_vs.shape == (num_obj,)
        return label_trans, label_rot, label_vs, label_class
